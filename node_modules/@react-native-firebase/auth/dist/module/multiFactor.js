...
export class MultiFactorUser {
  ...
  async enroll(multiFactorAssertion, displayName) {
    const assertion = multiFactorAssertion;
    if (assertion.factorId === 'phone') {
      await this._auth.native.finalizeMultiFactorEnrollment(assertion.token, assertion.secret, displayName ?? undefined);
    } else if (assertion.factorId === 'totp') {
      await this._auth.native.finalizeTotpEnrollment(assertion.totpSecret, assertion.verificationCode, displayName ?? undefined);
    } else {
    ...
    await reload(this._auth.currentUser);
  }
